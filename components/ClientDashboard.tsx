...
export const ClientDashboard: React.FC<Props> = ({ user: initialUser, onLogout }) => {
  ...
  const refreshUser = async () => {
    try {
      // Type casting since getById returns generic UsuarioConectado, we assume it implies Paciente fields for this user
      const updated = await api.users.getById(user.id);
      setUser(updated as PacienteConectado);
    } catch (e) {
  ...
  // Meet Link Component... (Same as before)
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 md:py-12 sm:px-6">
        ...
        <div className="relative space-y-8 md:space-y-12 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:ml-8 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-slate-200 before:via-slate-200 before:to-transparent z-0">
          {sesiones.map((sesion, index) => {
            ...
            const showMeetAfter = sesion.orden === 1;
            const showMeetBefore = sesion.orden === 4;
                  ...
                  <div className="p-5 md:p-7 bg-white rounded-[20px]">
                    ...
                    {!isLocked && (
                      <div className="space-y-3">
                        {showMeetBefore && <MeetCard title="Sesión de Cierre" sub="Reunión final con tu especialista antes del test." />}
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {sesion.recursos?.map(recurso => {
                            if (recurso.tipo === 'AUDIO') {
                              ...
                              return (
                                <div key={recurso.id} className="group relative overflow-hidden p-4 rounded-2xl bg-slate-900 text-white shadow-lg flex items-center justify-center transition-all hover:shadow-xl hover:-translate-y-0.5 cursor-pointer" onClick={() => setAudioPlaying(isPlaying ? null : recurso.id)}>
                                  {/* Condensed Audio UI for replacement brevity */}
                                  <div className="relative z-10 flex items-center gap-4 w-full">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-colors ${isPlaying ? 'bg-white text-slate-900' : 'bg-white/10 text-white'}`}>
                                      {isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}
                                    </div>
                                    <div><p className="text-sm font-bold">{recurso.titulo}</p></div>
                                  </div>
                                </div>
                              )
                            }
                            return (
                              <button key={recurso.id}
                                onClick={() => recurso.tipo === 'TEST' ? setShowTest(true) : alert('Abriendo guía...')}
                                className="flex items-center gap-4 p-3 md:p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-slate-200 hover:shadow-md transition-all group text-left w-full">
                                ...
                                <div className="flex-1">
                                  <span className="block text-sm font-bold text-slate-700">{recurso.titulo}</span>
                        ...
                        {showMeetAfter && <MeetCard title="Sesión de Inicio" sub="Reunión con tu especialista para revisar avances." />}
                      </div>
                    )}
                    {isLocked && (
        ...
        {showTest && (
          <TestModule
            program={nombrePrograma?.includes('CULPA') ? 'CULPA' : nombrePrograma?.includes('ANGUSTIA') ? 'ANGUSTIA' : 'IRRITABILIDAD'}
            weekNumber={semanaCalculada}
            userId={user.id}
