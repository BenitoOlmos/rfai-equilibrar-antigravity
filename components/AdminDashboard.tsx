...
const AdminUserManagement: React.FC<AdminUserManagementProps> = ({ roleFilter, title }) => {
    // API State
    const [users, setUsers] = useState<UsuarioConectado[]>([]);
    ...
    useEffect(() => {
        const loadUsers = async () => {
            setLoading(true);
            try {
                const fetched = await api.users.getAll();
                setUsers(fetched);
            } catch (err) {
        ...
        loadUsers();
    }, [roleFilter]); // Reload if needed, though getAll fetches all. Optimization possible.
    ...
    const handleDelete = (id: string) => {
        if (confirm('¿Confirma la eliminación de este usuario? Esta acción no se puede deshacer.')) {
            // Optimistic Dashboard Update (In real app, API DELETE call)
            // api.users.delete(id)
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        // Logic to link program if Client logic logic... API handles complex creation better usually.
        // For now, mapping same logic to frontend payload for create endpoint

        const payload = {
            name: newUser.name,
            email: newUser.email,
            role: newUser.role === RolSlug.ADMIN ? 'ADMIN' : newUser.role === RolSlug.COORDINATOR ? 'COORDINATOR' : newUser.role === RolSlug.PROFESSIONAL ? 'PROFESSIONAL' : 'CLIENT',
            // ... logic for profile data creation needs endpoint update in backend to support full profile on create, or two calls. 
            // Our backend create user only supports name/email/role. 
            // We'll simplify and do API call then local update for MVP or assume backend updated.
        };

        try {
            const created = await api.users.create(payload);
            setUsers([...users, created]); // Add optimistic or re-fetch
            setIsCreating(false);
